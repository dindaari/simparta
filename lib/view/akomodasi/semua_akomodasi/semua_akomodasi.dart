...
class SemuaAkomodasiScreen extends StatefulWidget {
  const SemuaAkomodasiScreen({Key? key}) : super(key: key);

  @override
  State<SemuaAkomodasiScreen> createState() => _SemuaAkomodasiScreenState();
}

class _SemuaAkomodasiScreenState extends State<SemuaAkomodasiScreen> {
  List<dynamic> dataAkomodasi = [];
  bool isLoading = true;
  bool hasError = false;
  String errorMessage = '';
  ...
  Future<void> fetchAkomodasiData() async {
    final url = Uri.parse(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> akomodasiData = data.where((item) {
          return item['obyek_jenis'] == 'akomodasi';
        }).toList();

        setState(() {
          dataAkomodasi = akomodasiData;
          isLoading = false;
        });
      } else {
        throw Exception(
    ...
    return Padding(
      ...
      child: GridView.builder(
        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
          ...
          crossAxisCount: 2, // Mengatur jumlah kolom menjadi 1
          mainAxisSpacing: 20.h,
          crossAxisSpacing: 20.h,
        ),
        shrinkWrap: true,
        physics: NeverScrollableScrollPhysics(), // Mengatasi overflow
        itemCount: dataAkomodasi.length,
        itemBuilder: (context, index) {
          final item = dataAkomodasi[index];
  ...
  Widget _buildGridItem(dynamic item) {
    return GestureDetector(
      onTap: () {
        ...
        //   context,
        //   MaterialPageRoute(
        //     builder: (context) => AkomodasiTentangScreen(
