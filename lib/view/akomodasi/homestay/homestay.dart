...
class _HomestayScreenState extends State<HomestayScreen> {
  ...
  Future<void> fetchHomeStayData() async {
    final url = Uri.parse(
        'http://demo.technophoria.co.id/simparta_pekalongan/api/rest_data?thisref=obyek&kategori_sub_id=13');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> homeStayData = data.where((item) {
          return item['obyek_id'] != null; // Pastikan ada 'obyek_id'
        }).toList();

        setState(() {
          dataHomeStay = homeStayData;
          isLoading = false;
        });
      } else {
        throw Exception(
  ...
  Widget _buildGridSection() {
    return Padding(
      ...
      child: GridView.builder(
        ...
        itemCount: dataHomeStay.length,
        itemBuilder: (context, index) {
          final item = dataHomeStay[index];
