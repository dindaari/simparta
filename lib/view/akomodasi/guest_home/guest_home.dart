...
class _GuestHomeScreenState extends State<GuestHomeScreen> {
  ...
  Future<void> fetchGuestHomeData() async {
    final url = Uri.parse(
        'http://demo.technophoria.co.id/simparta_pekalongan/api/rest_data?thisref=obyek&kategori_sub_id=25');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> guestHomeData = data.where((item) {
          return item['obyek_id'] != null; // Pastikan ada 'obyek_id'
        }).toList();

        setState(() {
          dataGuestHome = guestHomeData;
          isLoading = false;
        });
      } else {
        throw Exception(
  ...
  Widget _buildGridSection() {
    return Padding(
      ...
      child: GridView.builder(
        ...
        itemCount: dataGuestHome.length,
        itemBuilder: (context, index) {
          final item = dataGuestHome[index];
