...
class ArsitekturScreen extends StatefulWidget {
  const ArsitekturScreen({Key? key}) : super(key: key);
...
class _ArsitekturScreenState extends State<ArsitekturScreen> {
  ...
  Future<void> fetchArsitekData() async {
    final url = Uri.parse(
        'http://demo.technophoria.co.id/simparta_pekalongan/api/rest_data?thisref=obyek&kategori_sub_id=15');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> arsitekData = data.where((item) {
          return item['obyek_id'] != null; // Pastikan ada 'obyek_id'
        }).toList();

        setState(() {
          dataArsitek = arsitekData;
          isLoading = false;
        });
      } else {
        throw Exception(
  ...
  Widget _buildGridSection() {
    return Padding(
      ...
      child: GridView.builder(
        ...
        itemCount: dataArsitek.length,
        itemBuilder: (context, index) {
          final item = dataArsitek[index];
