...
class AplikasiPengembanganPermainan extends StatefulWidget {
  const AplikasiPengembanganPermainan({Key? key}) : super(key: key);

  @override
  State<AplikasiPengembanganPermainan> createState() => _AplikasiPengembanganPermainanState();
}

class _AplikasiPengembanganPermainanState extends State<AplikasiPengembanganPermainan> {
  List<dynamic> dataApp = [];
  bool isLoading = true;
  bool hasError = false;
  String errorMessage = '';
  ...
  Future<void> fetchAppData() async {
    final url = Uri.parse(
        'http://demo.technophoria.co.id/simparta_pekalongan/api/rest_data?thisref=obyek&kategori_sub_id=22');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> appData = data.where((item) {
