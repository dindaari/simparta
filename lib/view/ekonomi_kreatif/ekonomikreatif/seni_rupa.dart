...
class SeniRupaScreen extends StatefulWidget {
  const SeniRupaScreen({Key? key}) : super(key: key);

  @override
  State<SeniRupaScreen> createState() => _SeniRupaScreenState();
}

class _SeniRupaScreenState extends State<SeniRupaScreen> {
  List<dynamic> dataSeniRupa = [];
  bool isLoading = true;
  bool hasError = false;
  String errorMessage = '';
  ...
  Future<void> fetchSeniRupaData() async {
    final url = Uri.parse(
        'http://demo.technophoria.co.id/simparta_pekalongan/api/rest_data?thisref=obyek&kategori_sub_id=37');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> seniRupaData = data.where((item) {
          return item['obyek_id'] != null; // Pastikan ada 'obyek_id'
        }).toList();

        setState(() {
          dataSeniRupa = seniRupaData;
          isLoading = false;
        });
      } else {
        throw Exception(
  ...
  Widget _buildGridSection() {
    return Padding(
      ...
      child: GridView.builder(
        ...
        itemCount: dataSeniRupa.length,
        itemBuilder: (context, index) {
          final item = dataSeniRupa[index];
