...
class KriyaScreen extends StatefulWidget {
  const KriyaScreen({Key? key}) : super(key: key);

  @override
  State<KriyaScreen> createState() => _KriyaScreenState();
}

class _KriyaScreenState extends State<KriyaScreen> {
  List<dynamic> dataKriya = [];
  bool isLoading = true;
  bool hasError = false;
  String errorMessage = '';
  ...
  Future<void> fetchKriyaData() async {
    final url = Uri.parse(
        'http://demo.technophoria.co.id/simparta_pekalongan/api/rest_data?thisref=obyek&kategori_sub_id=19');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> kriyaData = data.where((item) {
  ...
  Widget _buildGridSection() {
    return Padding(
      ...
      child: GridView.builder(
        ...
        itemCount: dataKriya.length,
        itemBuilder: (context, index) {
          final item = dataKriya[index];
