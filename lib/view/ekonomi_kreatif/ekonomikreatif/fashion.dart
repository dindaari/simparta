...
class _FashionScreenState extends State<FashionScreen> {
  ...
  Future<void> fetchFashionData() async {
    final url = Uri.parse(
        'http://demo.technophoria.co.id/simparta_pekalongan/api/rest_data?thisref=obyek&kategori_sub_id=16');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> fashionData = data.where((item) {
          return item['obyek_id'] != null; // Pastikan ada 'obyek_id'
        }).toList();

        setState(() {
          dataFashion = fashionData;
          isLoading = false;
        });
      } else {
        throw Exception(
  ...
  Widget _buildGridSection() {
    return Padding(
      ...
      child: GridView.builder(
        ...
        itemCount: dataFashion.length,
        itemBuilder: (context, index) {
          final item = dataFashion[index];
