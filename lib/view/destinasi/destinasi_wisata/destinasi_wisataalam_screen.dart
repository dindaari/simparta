...
class WisataAlamScreen extends StatefulWidget {
  const WisataAlamScreen({Key? key}) : super(key: key);

  @override
  State<WisataAlamScreen> createState() => _WisataAlamScreenState();
}

class _WisataAlamScreenState extends State<WisataAlamScreen> {
  List<dynamic> dataWisataAlam = [];
  bool isLoading = true;
  bool hasError = false;
  String errorMessage = '';
  ...
  Future<void> fetchWisataAlamData() async {
    final url = Uri.parse(
        'http://demo.technophoria.co.id/simparta_pekalongan/api/rest_data?thisref=obyek&kategori_sub_id=1');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> wisataAlamData = data.where((item) {
          return item['obyek_id'] != null; // Pastikan ada 'obyek_id'
        }).toList();

        setState(() {
          dataWisataAlam = wisataAlamData;
          isLoading = false;
        });
      } else {
        throw Exception(
  ...
  Widget _buildGridSection() {
    return Padding(
      ...
      child: GridView.builder(
        ...
        itemCount: dataWisataAlam.length,
        itemBuilder: (context, index) {
          final item = dataWisataAlam[index];
