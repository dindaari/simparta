...
class WisataBudayaScreen extends StatefulWidget {
  const WisataBudayaScreen({Key? key}) : super(key: key);

  @override
  State<WisataBudayaScreen> createState() => _WisataBudayaScreenState();
}

class _WisataBudayaScreenState extends State<WisataBudayaScreen> {
  List<dynamic> dataWisataBudaya = [];
  bool isLoading = true;
  bool hasError = false;
  String errorMessage = '';
  ...
  void initState() {
    super.initState();
    fetchWisataBudayaData();
  }

  Future<void> fetchWisataBudayaData() async {
    final url = Uri.parse(
        'http://demo.technophoria.co.id/simparta_pekalongan/api/rest_data?thisref=obyek&kategori_sub_id=5');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> wisataBudayaData = data.where((item) {
          return item['obyek_id'] != null; // Pastikan ada 'obyek_id'
        }).toList();

        setState(() {
          dataWisataBudaya = wisataBudayaData;
          isLoading = false;
        });
      } else {
        throw Exception(
  ...
  Widget _buildGridSection() {
    return Padding(
      ...
      child: GridView.builder(
        ...
        itemCount: dataWisataBudaya.length,
        itemBuilder: (context, index) {
          final item = dataWisataBudaya[index];
