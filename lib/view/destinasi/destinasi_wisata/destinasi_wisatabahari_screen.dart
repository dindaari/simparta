...
class WisataBahariSceen extends StatefulWidget {
  const WisataBahariSceen({Key? key}) : super(key: key);

  @override
  State<WisataBahariSceen> createState() => _WisataBahariSceenState();
}

class _WisataBahariSceenState extends State<WisataBahariSceen> {
  List<dynamic> dataWisataBahari = [];
  bool isLoading = true;
  bool hasError = false;
  String errorMessage = '';
  ...
  void initState() {
    super.initState();
    fetchWisataBahariData();
  }

  Future<void> fetchWisataBahariData() async {
    final url = Uri.parse(
        'http://demo.technophoria.co.id/simparta_pekalongan/api/rest_data?thisref=obyek&kategori_sub_id=2');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> wisataBahariData = data.where((item) {
          return item['obyek_id'] != null; // Pastikan ada 'obyek_id'
        }).toList();

        setState(() {
          dataWisataBahari = wisataBahariData;
          isLoading = false;
        });
      } else {
        throw Exception(
  ...
  Widget _buildGridSection() {
    return Padding(
      ...
      child: GridView.builder(
        ...
        itemCount: dataWisataBahari.length,
        itemBuilder: (context, index) {
          final item = dataWisataBahari[index];
