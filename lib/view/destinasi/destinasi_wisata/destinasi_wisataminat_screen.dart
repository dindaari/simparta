...
class WisataMinatKhusus extends StatefulWidget {
  const WisataMinatKhusus({Key? key}) : super(key: key);

  @override
  State<WisataMinatKhusus> createState() => _WisataMinatKhususState();
}

class _WisataMinatKhususState extends State<WisataMinatKhusus> {
  List<dynamic> dataWisataMinat = [];
  bool isLoading = true;
  bool hasError = false;
  String errorMessage = '';
  ...
  void initState() {
    super.initState();
    fetchWisataMinatData();
  }

  Future<void> fetchWisataMinatData() async {
    final url = Uri.parse(
        'http://demo.technophoria.co.id/simparta_pekalongan/api/rest_data?thisref=obyek&kategori_sub_id=10');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> wisataMinatData = data.where((item) {
          return item['obyek_id'] != null; // Pastikan ada 'obyek_id'
        }).toList();

        setState(() {
          dataWisataMinat = wisataMinatData;
          isLoading = false;
        });
      } else {
        throw Exception(
  ...
  Widget _buildGridSection() {
    return Padding(
      ...
      child: GridView.builder(
        ...
        itemCount: dataWisataMinat.length,
        itemBuilder: (context, index) {
          final item = dataWisataMinat[index];
