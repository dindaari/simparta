...
class WisataBuatanScreen extends StatefulWidget {
  const WisataBuatanScreen({Key? key}) : super(key: key);

  @override
  State<WisataBuatanScreen> createState() => _WisataBuatanScreenState();
}

class _WisataBuatanScreenState extends State<WisataBuatanScreen> {
  List<dynamic> dataWisataBuatan = [];
  bool isLoading = true;
  bool hasError = false;
  String errorMessage = '';
  ...
  void initState() {
    super.initState();
    fetchWisataBuatanData();
  }

  Future<void> fetchWisataBuatanData() async {
    final url = Uri.parse(
        'http://demo.technophoria.co.id/simparta_pekalongan/api/rest_data?thisref=obyek&kategori_sub_id=4');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> wisataBuatanData = data.where((item) {
          return item['obyek_id'] != null; // Pastikan ada 'obyek_id'
        }).toList();

        setState(() {
          dataWisataBuatan = wisataBuatanData;
          isLoading = false;
        });
      } else {
        throw Exception(
  ...
  Widget _buildGridSection() {
    return Padding(
      ...
      child: GridView.builder(
        ...
        itemCount: dataWisataBuatan.length,
        itemBuilder: (context, index) {
          final item = dataWisataBuatan[index];
